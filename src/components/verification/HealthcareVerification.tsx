...
const HealthcareVerification: React.FC<HealthcareVerificationProps> = ({ apiKey, onResult }) => {
  ...
  const performVerification = async () => {
    ...
    try {
        ...
        const mockResult = {
          ...
          response: {
            ...
            healthcareInfo: {
              ...
              validUpto: '2025-06-14',
              authority: serviceId === 'medical-license' ? 'Medical Council of India' : 
                        serviceId === 'insurance-verification' ? 'IRDAI' : 'State Drug Control',
              status: 'Active',
              category: serviceData.specialization || serviceData.permit_type || 'General'
            }
          }
        };
      ...
      toast.success(`${selectedServices.length} healthcare verification(s) completed successfully`);
      setSelectedServices([]);
      setFormData({});
      
    } catch (error) {
      toast.error("Healthcare verification failed. Please try again.");
    } finally {
      setIsLoading(false);
    }
  };
